...
/**
 * Build the password-reset email. Mirrors the verification email's table-based,
 * inline-styled layout for maximum email-client compatibility.
 */
export function buildPasswordResetEmail(opts: {
  name: string;
  token: string;
}) {
  const url = `${APP_URL}/auth/reset-password?token=${encodeURIComponent(
    opts.token
  )}`;
  const subject = "Reset your RideBuddy password";
  const text = [
    `Hi ${opts.name},`,
    "",
    "We received a request to reset your RideBuddy password. Open the link below to choose a new one:",
    "",
    url,
    "",
          ...
          <table role="presentation" width="520" cellpadding="0" cellspacing="0" border="0" style="max-width:520px; width:100%; background:#ffffff; border-radius:14px; box-shadow:0 1px 3px rgba(15,31,26,0.06); overflow:hidden;">
            <tr>
              <td align="center" style="padding:36px 32px 8px 32px; font-family:${FONT};">
                ...
                <p style="margin:0; padding:0 8px; font-family:${FONT}; font-size:15px; line-height:1.55; color:#4b5563; text-align:center;">
                  Hi ${escapeHtml(opts.name)}, we got a request to reset your<br />
                  RideBuddy password. Tap the button below to choose a new one.
                </p>
              </td>
            </tr>
